...
const TaskCard = ({ task }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border-l-2 border-yellow-400 p-4">
      <div className="flex items-center mb-3">
        <img 
          src={getImageUrl(task.responsavel_foto_perfil[0])} 
          alt={task.responsavel_nome[0]}
          className="w-10 h-10 rounded-full object-cover ring-2 ring-white mr-3"
          onError={(e) => {
        ...
        <div className="ml-auto">
          <span className="inline-block bg-yellow-400 text-gray-800 text-xs px-2 py-1 rounded-full font-medium">
