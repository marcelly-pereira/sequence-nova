    ...
    try {
      ...
      const mockData = {
        Federais: [
          ...
          { name: 'IRPJ', status: 'Envio' },
          { name: 'CSLL', status: 'Envio' },
          { name: 'Contribuição Social sobre o Lucro Líquido (CSLL)', status: 'Envio' },
          { name: 'Programa de Integração Social (PIS)', status: 'Envio' },
          { name: 'Contribuição para o Financiamento da Seguridade Social (COFINS)', status: 'Envio' }
        ],
      };
      ...
      <div className="min-h-screen">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <h3 className="mb-2 mt-4 font-semibold text-sm pt-8">Quantidade de funcionários Inicial</h3>
            <div className="flex justify-between py-1">
            ...
            <div className="flex justify-between py-1">
              <span className="text-gray-700 text-sm">Quantidade de funcionário final</span>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="max-h-64 overflow-y-auto pr-1">
              {loading.obligations ? (
                ...
                <div>
                  {obligationsData[activeObligationCategory].map((obligation, index) => (
                    <div key={index} className="flex justify-between items-center p-3 border-b border-gray-200">
                      <span className="text-gray-700 text-sm">{obligation.name}</span>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="mb-4 overflow-x-auto">
              <table className="w-full min-w-full">
                <thead>
                  <tr>
                    <th className="font-medium text-sm bg-[#18183A] text-white p-2 text-left whitespace-nowrap rounded-tl-lg">CERTIDÕES</th>
                    <th className="font-medium text-sm bg-[#18183A] text-white p-2 text-left whitespace-nowrap">SITUAÇÃO</th>
                    <th className="font-medium text-sm bg-[#18183A] text-white p-2 text-left whitespace-nowrap rounded-tr-lg">DATA DE VENC.</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="p-2 border-b whitespace-nowrap text-sm">CND FEDERAL</td>
                    <td className="p-2 border-b">
                      <span className="bg-[#3DD35E] text-white px-4 py-1 rounded-full block text-center text-sm">Envio</span>
                    </td>
                    <td className="p-2 border-b whitespace-nowrap text-sm">15/08/2025</td>
                  </tr>
                  <tr>
                    <td className="p-2 border-b whitespace-nowrap text-sm">CND ESTADUAL</td>
                    <td className="p-2 border-b">
                      <span className="bg-[#3DD35E] text-white px-4 py-1 rounded-full block text-center text-sm">Envio</span>
                    </td>
                    <td className="p-2 border-b whitespace-nowrap text-sm">22/09/2025</td>
                  </tr>
                  <tr>
                    <td className="p-2 border-b whitespace-nowrap text-sm">CND TRABALHISTA</td>
                    <td className="p-2 border-b">
                      <span className="bg-[#3DD35E] text-white px-4 py-1 rounded-full block text-center text-sm">Envio</span>
                    </td>
                    <td className="p-2 border-b whitespace-nowrap text-sm">30/07/2025</td>
                  </tr>
                  <tr>
                    <td className="p-2 border-b whitespace-nowrap text-sm">CND MUNICIPAL</td>
                    <td className="p-2 border-b">
                      <span className="bg-[#3DD35E] text-white px-4 py-1 rounded-full block text-center text-sm">Envio</span>
                    </td>
                    <td className="p-2 border-b whitespace-nowrap text-sm">02/08/2004</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mb-4">
              <div className="text-md font-medium mb-3">ODOS ACELERADORA LTDA - 12</div>
              <div className="grid grid-cols-3 w-full">
                <button
                  className={`py-2 rounded-tl-lg ${activeFiscalCategory === 'Federal' ? 'bg-[#18183A] text-white font-medium text-sm' : 'bg-gray-100 text-gray-700 text-sm'}`}
                  onClick={() => setActiveFiscalCategory('Federal')}
                >
                  Federal
                </button>
                <button
                  className={`py-2 ${activeFiscalCategory === 'Estadual' ? 'bg-[#18183A] text-white font-medium text-sm' : 'bg-gray-100 text-gray-700 text-sm'}`}
                  onClick={() => setActiveFiscalCategory('Estadual')}
                >
                  Estadual
                </button>
                <button
                  className={`py-2 rounded-tr-lg ${activeFiscalCategory === 'Municipal' ? 'bg-[#18183A] text-white font-medium text-sm' : 'bg-gray-100 text-gray-700 text-sm '}`}
              ...
              <div className=" p-3">
                <div className="grid grid-cols-2 mb-1 pb-2 border-b border-gray-200">
                  <div className="font-semibold text-gray-800 text-sm">OBRIGAÇÃO</div>
                  <div className="font-semibold text-gray-800 text-right text-sm">VALOR</div>
                </div>
                ...
                <div className="max-h-52 overflow-y-auto">
                  {loading.fiscal ? (
                    ...
                    fiscalData[activeFiscalCategory].map((item, index) => (
                      <div key={index} className="grid grid-cols-2 py-2 border-b border-gray-200">
                        <div className="text-gray-700 text-sm">{item.name}</div>
                        <div className="text-gray-700 text-sm text-right">{item.value}</div>
                      </div>
                    ))
                  ) : (
                    <div className="py-4 text-center">
